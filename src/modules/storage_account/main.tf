...
resource "azurerm_private_dns_zone" "this" {
  count = var.storage_account_create ? length(local.dns_zones) : 0
...
resource "azurerm_private_dns_zone_virtual_network_link" "this" {
  count = var.storage_account_create ? length(azurerm_private_dns_zone.this) : 0
...
resource "azurerm_private_endpoint" "this" {
  ...
  name                = format("%s-%s", var.storage_account_private_endpoint_name, local.subresources[count.index])
  location            = var.resource_group_location
  resource_group_name = var.resource_group_name
